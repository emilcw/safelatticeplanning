...
"""
*** OBSERVE ***: 
This scrips is used to to an ablation study on the different improvements on the Safe Lattice Planner.
Thus this only works when running the lattice_planner_improved. 

See ablation_modes to toggle on/off the improvements and test them separately.
...
def simulation(motion_planner, image_name, world, mode, start_pose, test, scenario, scenario_parameter, location_tolerance, dji_safety_radius, ablation_mode):
    ...
    if(motion_planner == 'lattice_planner'):
        ...
        # Ablation modes
        adaptive_planning = ablation_mode['adaptive_planning']
        survival_planning = ablation_mode['survival_planning']
        emergency_trajectories = ablation_mode['emergency_trajectories']
        
        DOCKER_RUN_CMD_CORE = "./dev_env.sh start {0} /home/{0}/start_scripts/start_lattice_planner.sh {1} {2} {3} {4} {5}".format(image_name, location_tolerance[image_name], dji_safety_radius, adaptive_planning, survival_planning, emergency_trajectories)
        subprocess.run(NEW_TAB_CMD.format(DOCKER_RUN_CMD_CORE), shell=True)
        time.sleep(sleep_time)
...
ablation_modes = {'mode1' : {'adaptive_planning'     : False,
                            'survival_planning'      : False,
                            'emergency_trajectories' : False},
                'mode2' : {'adaptive_planning'       : True,
                            'survival_planning'      : False,
                            'emergency_trajectories' : False},
                'mode3' : {'adaptive_planning'       : False,
                            'survival_planning'      : True,
                            'emergency_trajectories' : False},
                'mode4' : {'adaptive_planning'       : False,
                            'survival_planning'      : False,
                            'emergency_trajectories' : True},
                'mode5' : {'adaptive_planning'       : True,
                            'survival_planning'      : True,
                            'emergency_trajectories' : False},
                'mode6' : {'adaptive_planning'       : False,
                            'survival_planning'      : True,
                            'emergency_trajectories' : True},
                'mode7' : {'adaptive_planning'       : True,
                            'survival_planning'      : False,
                            'emergency_trajectories' : True},
                'mode8' : {'adaptive_planning'       : True,
                            'survival_planning'      : True,
                            'emergency_trajectories' : True},
}
...
totalt_experiment_duration = run_time/60 * N_simulations * len(motion_planners) * len(worlds) * len(modes) * len(scenarios) * len(ablation_modes)
...
for ablation_key, ablation_mode in ablation_modes.items():
                ...
                for test in tests:
                    for scenario in scenarios:
                        for iter in range(N_simulations):
                            ...
                            success = simulation(motion_planner, image_name, world, mode, start_pose, test, scenario, scenario_parameter, location_tolerance, dji_safety_radius, ablation_mode)    
                            ...
                            if scenario in static_scenarios:
                                dst_folder = f"./experiment_data_sdmp/{ablation_key}/{motion_planner}_{world}_static_{test}_{scenario}_{iter}"
                            else:
                                dst_folder = f"./experiment_data_sdmp/{ablation_key}/{motion_planner}_{world}_dynamic_{test}_{scenario}_{iter}"
