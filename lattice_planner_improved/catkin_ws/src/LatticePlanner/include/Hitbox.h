...
public:
  ...
  bool is_inside(double x, double y, double z, double threshold = 0.0) {
    return position_min.x() <= x && x <= position_max.x() &&
           position_min.y() <= y && y <= position_max.y() &&
           (position_min.z()-threshold) <= z && z <= (position_max.z()+threshold);
  }
...
bool sphere2sphere_collision(Hitbox & h1, Hitbox & h2) {
  double dist = (h1.position - h2.position).squaredNorm(); 
  return dist < (h1.r + h2.r)*(h1.r + h2.r);
}
