...
class PhysicalEnvironment {
  ...
  public:
    ...
    bool in_allowed_volume(State * state, double threshold = 0.0);

    /* Collision checking */
    double evaluate_proximity_cost(Hitbox uav_hitbox, State * state, double start_time = 0.0, bool static_only = false, bool conservative = false);
    bool in_collision(Hitbox uav_hitbox, State * state, bool static_only = false, double start_time = 0.0, bool conservative = false);
    std::tuple<bool, double, TrajectoryState> in_collision(Hitbox uav_hitbox, std::vector<TrajectoryState> & trajectory, double start_time = 0.0, bool conservative = false);
