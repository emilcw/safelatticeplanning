...
inline double heuristic_fn2(State * s, State * goal) {
  //should be 1.0 but the planner gets slow without a little greed 
  return 5.0*(s->state.position - goal->state.position).norm(); 
  // 1.6 is fastest (and only) velocity for geometric primitives. 
  //*100 to make them less optimal when re-planning with increased dynamic horizon
}
