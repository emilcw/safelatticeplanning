...
if AVOIDANCE_ACTIVATED:
    rospy.loginfo("Drone avoidance mode is enabled.")
else:
	rospy.loginfo("Drone avoidance mode is disabled.")

state_sub = message_filters.Subscriber("/motion_planner_wrapper/model_states", ModelStates)
goal_sub = message_filters.Subscriber("/motion_planner_wrapper/goal", Goal)
...
dynamic_object_subscriber = rospy.Subscriber("/motion_planner_wrapper/model_states", ModelStates, update_human_positions)
...
pub = rospy.Publisher("/motion_planner_wrapper/set_model_state", ModelState, queue_size=10)

# Goal reached message publisher to planners
goal_reached_pub = rospy.Publisher("/motion_planner_wrapper/goal_reached", Bool, queue_size=1)

# Update avoidance mode and new avoidance goal
avoidance_pub = rospy.Publisher("/motion_planner_wrapper/avoidance", Bool, queue_size=1)
new_goal_pub = rospy.Publisher("/motion_planner_wrapper/new_goal", PoseStamped, queue_size=10)

# rviz
avoidance_goal_publisher = rospy.Publisher("/motion_planner_wrapper/goal_vector", Marker, queue_size=5)
