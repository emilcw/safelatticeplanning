...
REACHED_GOAL = False
EXPERIMENT_DATA_PATH = "/home/visualization/experiment_data_sdmp/"
modes = ["dynamic"] #"dynamic" "static"
...
A_STAR_LOW_RES = "A-star-low-res"
A_STAR_HIGH_RES = "A-star-high-res"
...
paths = {
    ...
    A_STAR_LOW_RES : table_path + "logfile_a_star_low_res.txt",
    A_STAR_HIGH_RES : table_path + "logfile_a_star_high_res.txt"
}
...
summary_paths = {
    ...
    A_STAR_LOW_RES : table_path + "summary_a_star_low_res.txt",
    A_STAR_HIGH_RES : table_path + "summary_a_star_high_res.txt"
}
...
def visualize_logfile_lp():
    ...
    for planner in [LATTICE_PLANNER, A_STAR_HIGH_RES, A_STAR_LOW_RES]:
        for world in worlds:
            for mode in modes:
                for test in tests:
                        ...
                        for scenario in scenarios:
                            ...
                            for iteration in range(simulation_runs):
                                ...
                                if os.path.isfile(file_path):
                                    ...
                                    collisions.append(df["dynamic_collisions"][0])
                                ...
                                if os.path.isfile(interval_path):
                                    ...
                                    if(not interval_df.empty and df["dynamic_collisions"][0] != 0):
                                        survival_times.append(interval_df["start-time"][0])
                                    else:
                                        """
                                        #Survived with no collisions
                                        #If it reached the goal, set the survival time to the simulation time
                                        #If it didn't reach the goal, set the survival time to 0
                                        average_time = None
                                        goal_reached_file_path = f"{EXPERIMENT_DATA_PATH}{planner}_{world}_{mode}_{test}_{scenario}_{iteration}/reached_goal.csv"
                                        if os.path.isfile(goal_reached_file_path):
                                            df_goal = pd.read_csv(goal_reached_file_path)
                                            reached_goal2 = bool(df_goal["GoalReached"][0])

                                        if reached_goal2:
                                            average_time = SIMULATION_TIME
                            ...
                            if at_least_one_iteration_exists:
                                ...
                                if len(survival_times) != 0: 
                                    ...
                                    survival_percent = (survival_time_mean / SIMULATION_TIME) * 100
                                else:
                                    #No collisions
                                    survival_time_mean = SIMULATION_TIME
                                    survival_time_std = 0
...
def visualize_summary_means(lp_improved_logfile, collision_df, improved_lp_summary_df, rrt_logfile, lp_logfile, rrt_summary_df, lp_summary_df):
    ...
    for planner in planners:
        ...
        if planner == LATTICE_PLANNER_IMPROVED:
            ...
            survival_percent = np.round(collision_df[collision_df['Motion Planner'] == planner]["Survival%"].mean(), DECIMALS)


        elif planner == LATTICE_PLANNER or planner == A_STAR_LOW_RES or planner == A_STAR_HIGH_RES:
            # Extract data from collision_df
            total_collision_duration_mean = np.round(collision_df[collision_df['Motion Planner'] == planner]["Total Collision duration (mean)"].mean(), DECIMALS)
            total_collision_duration_std = np.round(collision_df[collision_df['Motion Planner'] == planner]["Total Collision duration (mean)"].std(), DECIMALS)
            ...
            possible_goals = len(scenarios) * simulation_runs #len(lp_summary_df['Motion Planner'] == planner) * simulation_runs
        ...
        list_row = [planner, total_collision_duration_mean, total_collision_duration_std, collision_mean, collision_std, survival_time_mean, survial_time_std, planning_cycle_mean, planning_cycle_std, path_length_mean, path_length_std, int(reached_goals_sum), int(possible_goals), survival_percent]
